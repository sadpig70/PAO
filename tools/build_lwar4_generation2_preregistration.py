#!/usr/bin/env python3
"""Bind the live LWAR4 generation-2 identity before provider execution."""
...
TARGET = (
    REPO
    / "benchmarks"
    / "lwar4-generation2-calibration-preregistration-v1.json"
)
PREDECESSOR_PREREGISTRATION = (
    REPO / "benchmarks" / "lwar4-remediation-preregistration-v1.json"
)
CREATED_AT = "2026-07-27T02:56:00Z"
...
def build_preregistration(
    *,
    ...
    if identity.get("lwar_id") != "LWAR4" or identity.get("generation") != 2:
        raise RuntimeError("preregistration requires exact LWAR4 generation 2")
    profile = identity.get("profile")
    if not isinstance(profile, dict):
        raise RuntimeError("LWAR4 profile is missing")
    if (
        profile.get("adapter_id") != "qwen_code"
        or profile.get("vendor_family") != "alibaba"
        or profile.get("interface") != "cli"
    ):
        raise RuntimeError("LWAR4 profile is not the truthful Qwen CLI profile")
    ...
    predecessor_adapter = predecessor["adapter_contract"]
    retired_profile = {
        key: ADAPTERS["LWAR4"][key]
        for key in (
            "runtime_name",
            "model",
            "adapter_id",
    ...
    if (
        profile["adapter_id"] == retired_profile["adapter_id"]
        or profile["vendor_family"] == retired_profile["vendor_family"]
    ):
        raise RuntimeError("replacement provider duplicates retired generation 1")
    ...
    return {
        ...
        "preregistration_id": "lwar4-generation2-calibration-v1",
        ...
        "reset_action": {
            ...
            "reason": (
                "LWAR4 generation 2 passed 12/12 sealed recovery tasks with "
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "event": "generation2_preregistration_built",
