...
> Commands below use the in-repo form (`python PAO_plugin/scripts/...` with `--root .`). In operation mode, substitute `python "$PAO_HOME/scripts/..."` — or the plugin installation path revealed by the `lwar-runtime` skill when PAO is installed as a Claude Code plugin — and rely on `PAO_ROOT` instead of `--root .`.
...
```bash
python PAO_plugin/scripts/lwar.py register 1 \
  --runtime-name "Codex" \
  --model "GPT 5.5 Sol" \
  --adapter-id codex \
  --vendor-family openai \
...
```bash
python PAO_plugin/scripts/lwar.py complete \
  --identity-file <identity_file> \
  --task-id <task_id> \
...
```bash
python PAO_plugin/scripts/lwar.py state draining --identity-file <identity_file> --root .
python PAO_plugin/scripts/lwar.py state off --identity-file <identity_file> --root .
python PAO_plugin/scripts/lwar.py state on --identity-file <identity_file> --root .
python PAO_plugin/scripts/lwar.py state deregistered --identity-file <identity_file> --root .
```
