...
Since 0.3 the runtime is workspace-independent. Skill distribution is deliberately manual-first: users copy the OA and LWAR skill contracts (from `PAO_plugin/skills/`) into whichever global skills directory their runtime loads (`~/.agents/skills` is the emerging cross-runtime convention) — no installer required.

Since 0.4 the repository ships an installable Claude Code plugin at `PAO_plugin/` (manifest `PAO_plugin/.claude-plugin/plugin.json`, catalogued by the repository-root marketplace): skills, `/pao:*` command aliases, and the runtime install as one versioned unit, namespaced `pao:`. The `PAO_plugin/` directory is also the `PAO_HOME` target for manual installs. The skills reveal the plugin installation path via `${CLAUDE_PLUGIN_ROOT}`, replacing `PAO_HOME` in that channel. Manual copy remains the channel for non-Claude runtimes.

Also since 0.4, `PAO_skills/` provides a standalone, skills-only channel: `pao-oa` and `pao-lwar` are self-contained skills bundling the wrapper scripts and runtime (plus message schemas for the LWAR), installed by copying each folder into any global skills directory; only `PAO_ROOT` is required, and commands resolve paths through the `<PAO_SKILL>` placeholder (the folder containing the loaded SKILL.md). The bundled runtime copies are generated by `pao build-skills` from the canonical `PAO_plugin/` source and byte-verified against it by the test suite.
