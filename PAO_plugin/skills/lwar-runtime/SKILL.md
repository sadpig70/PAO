...
> **Bus root resolution**: every command resolves the bus as explicit `--root` > `PAO_ROOT` environment variable > a **`.pao/` folder under the current directory** (the default). The `.pao/` default keeps all PAO state (`mailbox/`, `var/`, `control/`) in one hidden folder instead of scattering it across the workspace — add `.pao/` to `.gitignore`. In operation mode set `PAO_ROOT` to a central bus and omit `--root`. Task execution still happens in each task's own `cwd`.
>
