...
- **Source of truth (permanent since 0.5.0)**: `PAO_skills/` is the canonical source; `PAO_skills/pao-lwar` is the runtime master. `PAO_plugin/` and `PAO_skills/pao-oa`'s runtime layer are **generated mirrors** — never edit them directly; edit the master, then run `python PAO_skills/sync_bundles.py --to-plugin`. Authored per-channel files (SKILL.md contracts, references, plugin manifest/commands/docs) are edited in place. The byte-mirror tests fail on any drift.
